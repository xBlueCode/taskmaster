    ...
    for prog_name in prog_names:
        if prog_name not in dashboard.programs.keys():
            ...
            for process in program.processes:
                ...
                if process.state != ProcessState.STARTING and \
                    process.state != ProcessState.BACKOFF and \
    ...
    for prog_name in prog_names:
        if prog_name not in dashboard.programs.keys():
            ...
            for process in program.process:
                utils.socket_send(cs, 'process {0} is in {1} state'.
                                  format(process.name, process.state))
                if process.state == ProcessState.STARTING or \
                        process.state == ProcessState.BACKOFF or \
                        process.state == ProcessState.RUNNING:
